...
const User = {
    ...
    register: function(User, callback) {
        ...
        db.query("SELECT* FROM user WHERE email = ?", [User.email], function(err, result, fields){
            if(result.length == 0) {
                return db.query("INSERT INTO user(firstName, surname, email, loanAmount, loanDuration, phoneNumber, password) VALUES(?,?,?,?,?,?,?)", [User.firstName, User.surname, User.email, User.loanAmount, User.loanDuration, User.phoneNumber, md5(User.password), dt], function(result, blabla) {
                    console.log(result)
                }. callback);
            }else {
